...
export default class AppStore {
    ...
    getErrorForNonEuClients = () => ({
        ...
        title: localize('Capital Edge is unavailable for this account'),
    ...
    getErrorForEuClients = (is_logged_in = false, country: string | undefined = undefined) => {
        return {
            text: ' ',
            title: is_logged_in
                ? localize(`Capital Edge is not available for ${country || 'EU'} clients`)
                : localize(`Capital Edge is unavailable in ${country || 'the EU'}`),
